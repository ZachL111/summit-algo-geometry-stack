...
`summit-algo-geometry-stack` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for geometry analysis with log and snapshot fixtures, replay consistency checks, and documented operating limits.
...
The score is deterministic. The current threshold is 175. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 4.
